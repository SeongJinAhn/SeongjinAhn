...
class AuthorParser( HTMLParser ):
    author_tag = "" #contains the last tag's name which point to author field
    m_Stop = False
    m_authors = []
    ...
    def handle_data(self, data):
        if self.m_Stop:
            return
        if self.tail_string == "Nothing":
            #print(data)  보안 문제로 안되네요 ㅠㅠㅠㅠㅠ
...
for index,paper in enumerate(all_papers):
    ...
    #AuthorPaser.feed(request.get(a).text)
